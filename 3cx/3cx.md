...
（一）安装环境
  ...
  在本地部署3CX，首先第一个要注意的就是IP地址的选择.这个请慎重，因为一旦决定，安装3CX服务器就不能改变IP地址了。除非，你打算重新部署。

[3CX系统部署IP地址的注意事项](aboutip.md)

  云端部署
  
  越来越多的用户选择在云端部署3CX。当然一定有同学会担心在云端部署的可靠性和通话质量问题，但是一个事实情况就是如今的网络情况已经不是10年前了，需要用发展的眼光来看问题，而且在未来，企业通信上云是大势所趋。

（二）安装需要准备的硬件环境
3CX可以部署在标准的X64结构的物理机，虚拟机和主流的云平台上。根据需求选择吧，不过物理机的备份和迁移，没有虚拟机和云平台管理方便。

[3CX部署的服务器平台配置](srv-install.md)

令人兴奋的是,3CX还可以部署在卡片电脑树莓派上。一个树莓派可以部署一套16并发,满足50个人使用的3CX。这一环境经过3CX官方测试超过半年。具有极客精神的朋友千万不要错过。

[树莓派运行3CX-IP PBX](raspbian-install.md)
